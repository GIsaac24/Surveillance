mod_overview_ui <- function(id) {
  ...
  tagList(
    section_title(
      "Vue d’ensemble",
      "Synthèse stratégique de la préparation et de la surveillance aux points d’entrée."
    ),
    uiOutput(ns("kpis")),
    fluidRow(
      column(6, div(class = "panel-card", plotOutput(ns("pillar_progress"), height = 430))),
...
mod_overview_server <- function(id, dashboard_data) {
  moduleServer(id, function(input, output, session) {
    ...
    output$pillar_progress <- renderPlot({
      dat <- req(dashboard_data())
      plot_pillar_progress(dat$pillar_activities)
    })
